...
var filter = m => m.author.id === message.author.id;
      ...
      message.channel.awaitMessages(filter, {
        ...
        if(!room) return message.channel.send('**Görünülürde böyle bir kanal bulamadım**');
        ...
        msg.edit('**Çekilişin süresini belirle (1s, 1m, 1h, 1d, 1w)**').then(msg => {
          message.channel.awaitMessages(filter, {
            ...
            if(!collected.first().content.match(/[1-60][s,m,h,d,w]/g)) return message.channel.send('**Görülünürülüşe göre böyle bir süre bilmiyorum**');
            ...
            msg.edit('**Şimdi de ödülü yaz bakalım**').then(msg => {
              message.channel.awaitMessages(filter, {
                ...
                try {
                  ...
                  message.guild.channels.cache.find(x => x.name ===  room).send('**ÇEKİLİŞ BAŞLADI**' , {embed: giveEmbed}).then(m => {
                     let re = m.react('🎉');
                     setTimeout(() => {
            ...
            .setFooter("RTX Bot çekiliş sistemi")
                ...
                message.channel.send(`**Maalesef gerekli yetkilerim bulunmamakta**`);
                  console.log(e);
                }
              });
            });
