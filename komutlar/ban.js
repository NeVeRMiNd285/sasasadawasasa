...
exports.run = async(client, message, args) => {
    ...
    let sebep = args.slice(1).join(' ') || "Belirtilmemiş."
     if(!user) return message.channel.send('<a:uyari:810237511405666375>     ``Bir kişi etiketlemelisin.``')
     if(user.id === message.author.id) return message.channel.send('<a:uyari:810237511405666375> ``Kendini banlayamazsın.``')
     if(user.id === client.user.id) return message.channel.send('<a:uyari:810237511405666375> ``Botu banlayamazsın.``')
  if(user.id === message.guild.ownerID) return message.channel.send('<a:uyari:810237511405666375>  ``Sunucu sahibini banlayamazsın.``')
    if (!message.guild.member(user).bannable) return message.reply('<a:uyari:810237511405666375>    ``Bu kişinin rolü senden üstte veya `Üyeleri yasakla` yetkisine sahip.``');
